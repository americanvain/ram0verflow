"""
ROFL cryptographic primitives. Pure Python standard library only.
...
Contains:
  ...
  - bech32             : BIP 173 encoding, human-readable part "rofl"
...
def pubkey_to_address(pub_bytes: bytes) -> str:
    """
    Address = bech32(hrp="rofl", version 0, sha256(pubkey)[:20]).

    Bitcoin hashes with RIPEMD160(SHA256(pk)); RIPEMD160 is absent from many
    modern OpenSSL builds, so ROFL truncates a single SHA-256 instead. The
